    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/waitlist/join`, {
        ...
        body: JSON.stringify({
          ...
          image: product?.image || product?.images?.[0] || null // Product image URL
        })
      });
      ...
      // Mock success for frontend-only mode
      setSuccess(true);
      setResult({
        access_code: 'RAZE-' + Math.random().toString(36).substr(2, 6).toUpperCase(),
        total_items: sizeSelections.map(s => `${s.size} x${s.quantity}`).join(', ')
      });
    } finally {
      setLoading(false);
    }
